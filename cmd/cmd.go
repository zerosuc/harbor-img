...
func NewHarborCleanCommand() *cobra.Command {
	...
	rootCmd.Flags().StringVarP(&url, "url", "u", "http://10.200.82.51", "例如：https://harbor.abc.com")
	rootCmd.Flags().StringVarP(&user, "user", "U", "admin", "用户名，例如：admin")
	rootCmd.Flags().StringVarP(&password, "password", "p", "Harbor12345", "密码")
	rootCmd.Flags().StringVarP(&projectName, "projectName", "P", "", "项目名，all表示所有项目")
	rootCmd.Flags().IntVarP(&keepNum, "keepNum", "k", 200, "保留的tag数目，例如50")
	return rootCmd
}
